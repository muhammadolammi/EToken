...
public interface ICustomerDeviceService
{
    ...
    Task<IEnumerable<CustomerDevice>> GetDevicesByCifAsync(Guid cif, CancellationToken ct = default);
    Task UpdateDeviceStatusAsync(Guid deviceId,string status, CancellationToken ct = default);

    Task<CustomerDevice> RegisterDeviceAsync(Guid cif,Guid deviceId,  string? deviceModel,  CancellationToken ct = default);
